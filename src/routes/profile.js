...
profileRouter.patch("/edit", authValidator, async (req, res, next) => {
  try {
    const user = req.user;
    const allowedForEdit = ["firstName", "lastName", "gender", "photoUrl", "about", "age", "skill"]
    const isEditAllowed = Object.keys(req.body).every((field) => allowedForEdit.includes(field));
    if (!isEditAllowed) {
...
profileRouter.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const _user = await User.updateOne({ emailId: emailId }, { password: hashedPassword })
